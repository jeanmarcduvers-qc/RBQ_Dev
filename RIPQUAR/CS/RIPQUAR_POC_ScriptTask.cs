...
namespace ST_ParseJSONTestWe
{
    ...
    public partial class ScriptMain : Microsoft.SqlServer.Dts.Tasks.ScriptTask.VSTARTScriptObjectModelBase
    {
        ...
        private void TruncateStagingTable()
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("TRUNCATE TABLE [RIPQUAR_STG].[STG_TW_EXAMEN]", conn))
                {
                    cmd.ExecuteNonQuery();
                }
            }
        ...
        private void ParseAndLoadExam(DataRow sourceRow)
        {
            ...
            string examGicId = sourceRow["PAR_ROW_ID__CXEXAMEN"].ToString();
            string contactGicId = sourceRow["PAR_ROW_ID__SCONTACT"].ToString();
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                ...
                string insertQuery = @"
                    INSERT INTO [RIPQUAR_STG].[STG_TW_EXAMEN]
                    (
                        TW_N_EXAM_ID,
                        TW_C_EXAM,
                        TW_Q_DURE_SEC,
                ...
                using (SqlCommand cmd = new SqlCommand(insertQuery, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@GicRowId", rowId);
                    cmd.Parameters.AddWithValue("@GicExamId", examGicId);
                    cmd.Parameters.AddWithValue("@GicContactId", contactGicId);
